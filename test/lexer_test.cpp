...
TEST(LexerTest, LexStringTest) {
  ...
  ASSERT_EQ(tokens[0].ToString(),
            "Token(Hello, World!) type::String family::Literal\n");
}
...
TEST(LexerTest, LexKeywordTest) {
  Lexer lexer;
  std::vector<Token> tokens =
      lexer.Lex("func for continue break return if else false true null "
                "undefined using from");
