...
unique_ptr<Assignment> GetStrAssign(string id, string value) {
  return make_unique<Assignment>(info, GetIdentifier(id), GetString(value));
}
...
unique_ptr<For> GetFor(StatementPtr &&decl, ExpressionPtr &&condition,
                       StatementPtr &&increment, BlockPtr &&block) {
...
StatementPtr GetIntAssign(string iden, int value) {
  ...
  return make_unique<Assignment>(info, std::move(id),
                                 std::make_unique<Operand>(info, zero));
}
...
ExpressionPtr GetLessThanExpr(string iden, int rvalue) {
  return make_unique<BinExpr>(
      info, GetIdentifier(iden),
      std::make_unique<Operand>(info, Int_T::New(rvalue)), TType::Less);
}
StatementPtr GetIncrement(string i) {
  return make_unique<UnaryStatement>(
      info, make_unique<UnaryExpr>(info, GetIdentifier(i), TType::Increment));
}
...
TEST(ASSIGN, ASSIGN_INT) {
  ...
  ASSERT_TRUE(ASTNode::context.Find("myvalue")->Equals(Int_T::New(100)));
}
TEST(ASSIGN, ASSIGN_FLOAT) {
  ASTNode::context.Reset();
  GetFloatAssign("myvalue", 100.0f)->Execute();
  ASSERT_TRUE(ASTNode::context.Find("myvalue")->Equals(Float_T::New(100.0f)));
}
TEST(ASSIGN, ASSIGN_ARRAY) {
  ASTNode::context.Reset();
  auto result =
      GetArrayAssign("myArray", {Int_T::New(0), Int_T::New(1)})->Execute();
...
TEST(ASSIGN, ASSIGN_OBJECT) {
  ASTNode::context.Reset();
  auto result =
      GetObjectAssign("myObject", GetBlock("myInnerValue", "SomeValue"))
          ->Execute();

  ASSERT_EQ(result.controlChange, ControlChange::None);

  auto object =
      static_cast<Object_T *>(ASTNode::context.Find("myObject").get());
...
TEST(CONTROL_FLOW, TEST_FOR_TRUE) {
  auto result =
      GetFor(nullptr, GetTrueExpr(), nullptr, GetBlockBreak())->Execute();
  ASSERT_TRUE(result.controlChange == ControlChange::None);
}
TEST(CONTROL_FLOW, TEST_FOR_I_LESS_THAN_10) {
  auto result = GetFor(GetIntAssign("i", 0), GetLessThanExpr("i", 10),
                       GetIncrement("i"), GetBlockBreak())
                    ->Execute();
  ASSERT_TRUE(result.controlChange == ControlChange::None);
...
TEST(CONTROL_FLOW, TEST_IF_WITH_ELSE) {
  ASTNode::context.Reset();
  GetStrAssign("myvalue", "myDefaultValue")->Execute();
  auto if_with_else = If::WithElse(
      info, GetFalseExpr(), GetBlock("myvalue", "VALUE_SET_IN_IF_BLOCK"),
...
TEST(CONTROL_FLOW, TEST_IF_FALSE_ELSE_IF_TRUE_ELSE) {
  ...
  */
  ...
  auto if_with_else_if = If::WithElse(
      ...
      Else::New(
          info,
          If::WithElse(info, GetTrueExpr(),
                       GetBlock("myvalue", "VALUE_SET_IN_ELSE_IF"),
                       Else::NoIf(info, GetBlock("myvalue",
                                                 "VALUE_SET_IN_FINAL_ELSE")))));

  if_with_else_if->Execute();

  ASSERT_TRUE(ASTNode::context.Find("myvalue")->Equals(
      String_T::New("VALUE_SET_IN_ELSE_IF")));
}
TEST(CONTROL_FLOW, TEST_IF_FALSE_ELSE_IF_FALSE_ELSE) {
  ...
  */
  ...
  ASSERT_TRUE(ASTNode::context.Find("myvalue")->Equals(
      String_T::New("VALUE_SET_IN_FINAL_ELSE")));
}
